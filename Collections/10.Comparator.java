...
class Checker implements Comparator<Player>{ 	//compare() method of Comparator<Player>; compareTo() of Comparable<Player> interface
 public int compare(Player p1, Player p2){
     Integer c;
     c=(((Integer)p1.score).compareTo(p2.score));		
     if( c != 0)                                    //if score of comparing two players are not same
         return -c;                                 // sort them, - sign to sort in decending order of score
     else                                           // if score of comparing two players are same (*even "else is not required , and works without else)
       return ( ((p1.name).compareTo(p2.name)) );   // sort that two players in ascending order of name
 }
