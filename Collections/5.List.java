...
public class Solution {
	...
	public static void main(String[] args) {
		...
		int mlc = Integer.parseInt(sin.next());	//main list count
				
		List<Integer> al = new ArrayList<Integer>();
		
		for(int i=0; i<mlc;i++){
			int ele = Integer.parseInt(sin.next());
			al.add(i,ele);						// input to arraylist based on index
		}
		
		int req = Integer.parseInt(sin.next());
		
		for(int r=0;r<req;r++){
			String s = sin.next();
			
			if(s.equals("Insert")){
				int x = Integer.parseInt(sin.next());
				int y = Integer.parseInt(sin.next());
